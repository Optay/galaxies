...
WAGNER.RGBSplitPass = function() {
    ...
    this.loadShader("rgb-split-fs.glsl",
        function() {
        });
...
WAGNER.ASCIIPass = function() {
    ...
    this.loadShader("ascii-fs.glsl",
        function() {
            this.shader.uniforms.tAscii.value =
                THREE.ImageUtils.loadTexture(WAGNER.assetsPath + "/ascii/8x16_ascii_font_sorted.gif");
        });
...
WAGNER.HalftonePass = function() {
    ...
    this.loadShader("halftone-fs.glsl",
        function() {
...
WAGNER.HalftoneCMYKPass = function() {
    ...
    this.loadShader("halftone-cmyk-fs.glsl",
        function() {
...
WAGNER.SimpleSSAOPass = function() {
    ...
    this.loadShader("ssao-simple-fs.glsl",
        function(fs) {
        });
...
WAGNER.DirectionalBlurPass = function() {
    ...
    this.loadShader("guided-directional-blur-fs.glsl",
        function(fs) {
...
WAGNER.BleachPass = function() {
    ...
    this.loadShader("bleach-fs.glsl",
        function(fs) {
...
WAGNER.WarpBubblePass = function() {
    WAGNER.Pass.call(this);

    this.shader = WAGNER.processShader(galaxies.shaders.postProcess.warpBubble.vertexShader,
        galaxies.shaders.postProcess.warpBubble.fragmentShader);

    this.params.center = new THREE.Vector2(0.5, 0.5);
    this.params.maxRadius = new THREE.Vector2(0.09, 0.16);
    this.params.warpFactor = 2.0;
    this.params.progression = 0;
};

WAGNER.WarpBubblePass.prototype = Object.create(WAGNER.Pass.prototype);

WAGNER.WarpBubblePass.prototype.run = function(c) {
    this.shader.uniforms.center.value.copy(this.params.center);
    this.shader.uniforms.maxRadius.value.copy(this.params.maxRadius);
    this.shader.uniforms.warpFactor.value = this.params.warpFactor;
    this.shader.uniforms.progression.value = this.params.progression;
