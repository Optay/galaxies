...
Object.defineProperty(galaxies.engine,
    "soundDilation",
    {
        get: function() {
...
galaxies.engine.POWERUP_CHARGED =
    3300; //3300; // powerup spawns when this many points are earned, set low for easier testing of powerups
...
galaxies.engine.VISIBLE_RADIUS = galaxies.engine.CAMERA_Z *
    galaxies.engine.CONE_SLOPE *
    galaxies.engine.CAMERA_SLOPE /
    (galaxies.engine.CONE_SLOPE + galaxies.engine.CAMERA_SLOPE);
...
Object.defineProperty(galaxies.engine,
    "levelNumber",
    {
        ...
        set: function(value) {
            ...
            galaxies.engine.planetNumber = Math.floor((galaxies.engine.levelNumber - 1) /
                    galaxies.engine.ROUNDS_PER_PLANET) +
                1;
        }
    });
...
galaxies.engine.updateView = function() {
    ...
    galaxies.engine.VISIBLE_RADIUS = galaxies.engine.CAMERA_Z *
        ...
        galaxies.engine.CAMERA_SLOPE /
        (galaxies.engine.CONE_SLOPE + galaxies.engine.CAMERA_SLOPE);
    galaxies.engine.OBSTACLE_VISIBLE_RADIUS = galaxies.engine.CAMERA_Z *
        galaxies.engine.CONE_SLOPE *
        cameraSlope /
        (galaxies.engine.CONE_SLOPE + cameraSlope);

    galaxies.Projectile.prototype.PROJECTILE_SPEED = galaxies.engine.CONE_SLOPE * 23; // 3.0 in original
    galaxies.Projectile.prototype.PROJECTILE_LIFE =
        (galaxies.engine.OBSTACLE_VISIBLE_RADIUS - galaxies.engine.PROJ_START_Y) /
        galaxies.Projectile.prototype.PROJECTILE_SPEED +
        0.1;
...
galaxies.engine.initTutorialData = function(playerData, storageAvailable) {
    ...
    Object.defineProperty(playerData,
        "completedTutorial",
        {
            get: function() {
...
galaxies.engine.initScene = function() {
    ...
    galaxies.engine.camera = new THREE.PerspectiveCamera(galaxies.engine.CAMERA_VIEW_ANGLE,
        galaxies.engine.canvasWidth / galaxies.engine.canvasHeight,
        0.3,
        1100);
    ...
    galaxies.engine.sunFlares = new THREE.LensFlare(flareTex,
        60,
        0.2,
    ...
    galaxies.engine.renderer.context.canvas.addEventListener("webglcontextlost",
        galaxies.engine.handleContextLost,
        false);
    galaxies.engine.renderer.context.canvas.addEventListener("webglcontextrestored",
        galaxies.engine.handleContextRestored,
        false);
    ...
    galaxies.audio.listener.setPosition(galaxies.audio.listenerObject.position.x,
        galaxies.audio.listenerObject.position.y,
        galaxies.audio.listenerObject.position.z);
...
galaxies.engine.initGame = function(startAtPlanet) {
    ...
    galaxies.engine.planet =
        new THREE.Mesh(galaxies.resources.geometries["moon"], galaxies.resources.materials["moon"]);
...
galaxies.engine.startGame = function(startAtPlanet) {
    ...
    if (hasStartPoint && !levelSupplied) {
        ...
        galaxies.engine.levelNumber = levelSupplied
            ? (startAtPlanet * galaxies.engine.ROUNDS_PER_PLANET + 1)
            : galaxies.engine.START_LEVEL_NUMBER;
    }
...
galaxies.engine.initLevel = function() {
    ...
    var planetFirstSpeed =
        1 + 1 / (1 + Math.exp(4 - galaxies.engine.planetNumber)); // Speed on first level for this planet
    var planetLastSpeed =
        1 + 1.5 / (1 + Math.exp(1 - galaxies.engine.planetNumber / 2)); // Speed on last level for this planet

    galaxies.engine.speedScale = THREE.Math.mapLinear(galaxies.engine.roundNumber,
        1,
        3,
        planetFirstSpeed,
        planetLastSpeed);
...
galaxies.engine.nextLevel = function() {
    ...
    if (galaxies.engine.roundNumber === 1) {
        ...
        galaxies.engine.roundScore =
            galaxies.utils.calculateRoundScore(rawScore, accuracy, galaxies.engine.starsCollected);
...
galaxies.engine.updateBackgroundPlanet = function() {
    ...
    console.log("Setting background planet texture",
        galaxies.engine.planetNumber,
        galaxies.resources.planetData.length,
        bgPlanetIndex);
...
galaxies.engine.startPlanetMove = function() {
    ...
    createjs.Tween.get(galaxies.engine.planet.position)
        .to({ x: outPosition.x, y: outPosition.y, z: outPosition.z },
            transitionTimeMilliseconds / 2,
            createjs.Ease.quadInOut)
        ...
        .call(function() {
                ...
                if (galaxies.engine.inTutorial) {
                    ...
                    console.log(galaxies.engine.planetNumber,
                        galaxies.resources.levelTitles[galaxies.engine.planetNumber - 1]);
                }
    ...
    // Tween time should be dependent on transition time which should be a constant
    createjs.Tween.get(galaxies.engine.bgPlanet.scale)
        .to({ x: targetScale, y: targetScale, z: targetScale },
            transitionTimeMilliseconds / 2,
            createjs.Ease.quadInOut);
...
galaxies.engine.updateScene = function() {
    ...
    if (galaxies.engine.roundNumber === 1) {
        ...
        // Update lighting
        galaxies.engine.setLightPosition(galaxies.resources.lightAngles[(galaxies.engine.planetNumber - 1) %
            galaxies.resources.lightAngles.length]);
    }

};
galaxies.engine.randomizePlanet = function() {
    galaxies.engine.planet.rotation.set(Math.random() * galaxies.utils.PI_2,
        Math.random() * galaxies.utils.PI_2,
        Math.random() * galaxies.utils.PI_2,
        "ZXY");
    galaxies.engine.planet.material.color.setHSL(Math.random(),
        THREE.Math.randFloat(0.1, 0.4),
        THREE.Math.randFloat(0.5, 0.7));
    galaxies.debug.moonColor.setValue("#" + galaxies.engine.planet.material.color.getHexString());
    galaxies.engine.planetAngle =
        galaxies.engine.planet.rotation.z; // planetAngle is the zero value for rotation the planet when lux moves
...
galaxies.engine.getProjectile = function(startAngle, directionOffset, indestructible, particles) {
    ...
    if (galaxies.engine.projectilePool.length > 0) {
        ...
        proj.initialize(galaxies.engine.currentPowerup,
            projMesh,
            startAngle,
            directionOffset,
            indestructible,
        ...
        proj = new galaxies.Projectile(galaxies.engine.currentPowerup,
            projMesh,
            startAngle,
            directionOffset,
            indestructible,
...
galaxies.engine.shoot = function(indestructible) {
    ...
    if (galaxies.engine.shotTimer > 0) {
        return;
    }
    galaxies.engine.shotTimer = galaxies.engine.SHOOT_TIME;
    ...
    var proj = galaxies.engine.getProjectile(galaxies.engine.angle,
        0,
        indestructible,
        indestructible ? galaxies.FX.GetRainbowEmitter() : null);
...
galaxies.engine.shoot3 = function() {
    ...
    for (var i = -1; i <= 1; i++) {
        ...
        var proj = galaxies.engine.getProjectile(galaxies.engine.angle,
            i * Math.PI / 6,
            false,
            galaxies.FX.GetSmallFlameJet(i === 0));
...
galaxies.engine.update = function() {
    ...
    if (delta > 0.25) {
        delta = 0.25;
    } // Cap simulation at 4 ticks/second delta to prevent projectiles from passing through objects.
    ...
    galaxies.engine.planeSweep.potentialCollisions().forEach(function(collisionPair) {
        ...
        if (numProjectiles === 0) {
            ...
            if (aIsObstacle && bIsObstacle) {
                ...
                if (laser.isHittable &&
                    obsA.object.position.distanceToSquared(obsB.object.position) <
                    obstacle.hitThreshold * obstacle.hitThreshold) {
                    laser.impact();
                    obstacle.hit(1);
                }
            }
    ...
    if (galaxies.engine.shotTimer > 0) {
        galaxies.engine.shotTimer -= delta;
    }
    ...
    if (galaxies.engine.sun.visible) {
        ...
        galaxies.engine.sunFlares.position.copy(galaxies.engine.sun.position.clone().sub(cameraScenePos)
            .multiplyScalar(0.5).add(cameraScenePos));
...
galaxies.engine.hitPlayer = function() {
    if (galaxies.engine.isGameOver) {
        return;
    } // prevent any rogue obstacles from causing double-death
...
galaxies.engine.gameOver = function(isWin) {
    ...
    galaxies.engine.roundScore =
        galaxies.utils.calculateRoundScore(galaxies.engine.roundScore, accuracy, galaxies.engine.starsCollected);

    galaxies.engine.score = galaxies.engine.previousTotal + galaxies.engine.roundScore;
    bonusScore = galaxies.engine.roundScore - rawScore;

    var sequence = createjs.Tween.get(null).wait(2000).call(galaxies.ui.updateScore, [galaxies.engine.score]);
    ...
    if (isWin) {
        ...
        sequence.call(galaxies.ui.showGameOver, [isWin, galaxies.engine.score, bonusScore, accuracy]);
    }
};
...
galaxies.engine.collectStar = function(fromObject) {
    galaxies.ui.animateCollection(galaxies.ui.createFloatingStar(),
        fromObject,
        galaxies.ui.getFirstEmptyStarPosition(),
        function() {
...
galaxies.engine.showCombo = function(value, multiplier, obj) {
    ...
    if (galaxies.engine.powerupCharge >= 1) {
        var giveHeart = (galaxies.engine.playerLife < galaxies.engine.MAX_PLAYER_LIFE) &&
            ((galaxies.engine.powerupCount % 4) === 0);
...
galaxies.start = function() {
    ...
    if (galaxies.utils.isMobile()) {
        ...
        ttsElement.addEventListener("click",
            function() {
                ttsElement.remove();
...
galaxies.engine.addBoss = function(bossType) {
    ...
    var titleText = "ROUND 4<br>" +
        galaxies.engine.boss.name.toUpperCase() +
        " vs " +
...
galaxies.engine.setPowerup = function(newPowerup, fromObject) {
    ...
    if (newPowerup === "heart") {
        if (galaxies.engine.playerLife < galaxies.engine.MAX_PLAYER_LIFE) {
            galaxies.ui.animateCollection(galaxies.ui.createFloatingHeart(),
                fromObject,
                galaxies.ui.getFirstEmptyHeartPosition(),
                function() {
                    galaxies.engine.playerLife++;
                    galaxies.ui.updateLife(galaxies.engine.playerLife);
                });
