...
this.galaxies.Player = function() {
    ...
    frames = galaxies.utils.generateSpriteFrames(new THREE.Vector2(4114, 0),
        new THREE.Vector2(256, 256),
        new THREE.Vector2(8192, 256),
        4,
        new THREE.Vector2(0, 0),
    ...
    frames = galaxies.utils.generateSpriteFrames(new THREE.Vector2(5698, 0),
        new THREE.Vector2(256, 256),
        new THREE.Vector2(8192, 256),
        4,
        new THREE.Vector2(3, 0),
    ...
    frames = galaxies.utils.generateSpriteFrames(new THREE.Vector2(5073, 0),
        new THREE.Vector2(256, 256),
        new THREE.Vector2(8192, 256),
        4,
        new THREE.Vector2(62, 0),
    ...
    var fireCloneProjectile = function() {
        ...
        var proj = new galaxies.Projectile("clone",
            projMesh,
            cloneAIData.angle,
            0,
            false,
            galaxies.FX.GetPurpleTrailEmitter());
    ...
    var getUFOFuturePosition = function(ufo, delta) {
        ...
        if (cloneAIData.ufo.previousPosition) {
            ...
            predictedPosition =
                new THREE.Vector3(-Math.sin(ufoFutureAngle), Math.cos(ufoFutureAngle), 0).multiplyScalar(distance);
        }
    ...
    var update = function(delta, angle) {
        ...
        if (cloneRotator.parent !== null && !teleportingClone && !spinningOutClone) {
            ...
            if (cloneAIData.targetObject) {
                ...
                cloneAIData.angle += Math.min(rotationalVelocity, Math.abs(cloneAngleDelta)) *
                    (cloneAngleDelta < 0 ? -1 : 1);
            }

            cloneAIData.angle = Math.min(Math.max(cloneAIData.angle, cloneDefaultAngle - cloneAIData.maxWanderAngle),
                cloneDefaultAngle + cloneAIData.maxWanderAngle);
        ...
        if (spinningOutClone) {
            ...
            if (normCloneScreenPos.x < -0.1 ||
                normCloneScreenPos.x > 1.1 ||
                normCloneScreenPos.y < -0.1 ||
                normCloneScreenPos.y > 1.1) {
                stopSpinningOutClone();
            }
        }
    };
    ...
    var teleportEffectComplete = function() {
        ...
        if (cloneTeleportSprite.parent) {
            cloneTeleportSprite.parent.remove(cloneTeleportSprite);
        }
        teleporting = false;
    };
    ...
    var setPowerup = function(powerup) {
        ...
        character.position.set(0,
            galaxies.engine.CHARACTER_POSITION,
            0); // note that character is offset horizontally because sprites are not centered
