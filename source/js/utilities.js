...
galaxies.utils.isMobile = function() {
    if (galaxies.utils._isMobile === null) {
        galaxies.utils._isMobile =
            /iPhone|iPad|iPod|Android|windows phone|iemobile|\bsilk\b/i.test(navigator.userAgent);
    }
...
galaxies.utils.normalizeAngle = function(angle) {
    angle = angle % galaxies.utils.PI_2;

    if (angle > Math.PI) {
        angle -= galaxies.utils.PI_2;
...
galaxies.utils.getSegmentCircleIntersections = function (circleOrigin, circleRadius, segmentStart, segmentEnd) {
    var segmentLocalStart = segmentStart.clone().sub(circleOrigin),
        segmentLocalEnd = segmentEnd.clone().sub(circleOrigin),
        m = (segmentLocalEnd.y - segmentLocalStart.y) / (segmentLocalEnd.x - segmentLocalStart.x),
        o = segmentLocalStart.y - m * segmentLocalStart.x,
        a = m * m + 1,
        b = 2 * m * o,
        c = o * o - circleRadius * circleRadius,
        radicalTerm = b * b - 4 * a * c,
        doubleA = 2 * a;
    ...
    var points = [(-b + radicalTerm) / doubleA];

    if (radicalTerm !== 0) {
        points.push((-b - radicalTerm) / doubleA);
    }

    return points.map(function (x) { return new THREE.Vector3(x, m * x + o, 0).add(circleOrigin); });
};
