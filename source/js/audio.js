...
galaxies.audio.DIRECTION_FOCUS =
    1; // How much sounds spread to neighboring speakers: higher values produces sharper spatialization, lower values spread sound more evenly
...
galaxies.audio.sounds = {
    ...
    'squishsplat': ["squishsplat"]
};

// Decode and package loaded audio data into exhaustive array objects.
galaxies.audio.initAudio = function(complete) {
    var AudioContext = window.AudioContext || window.webkitAudioContext;
    galaxies.audio.audioCtx =
        galaxies.audio.audioCtx ||
        new AudioContext(); // audio context will already have been defined in mobile touch-to-start event.
...
galaxies.audio.PositionedSound = function(props) {
    ...
    if (typeof(props.dispose) !== "boolean") {
        props.dispose = true;
    }
    ...
    if (typeof(props.baseVolume) !== "number") {
        props.baseVolume = 1;
    }
    ...
    this.updatePosition = function(newPosition) {
        ...
        if (galaxies.audio.surroundMix) {
            ...
            this.inPlaneWeight = 0;
            if (this.distance > 0) {
                this.inPlaneWeight = Math.sqrt(Math.pow(this.toSource.x, 2) + Math.pow(this.toSource.z, 2)) /
                    this.distance;
            }
            for (var iOutput = 0; iOutput < 6; iOutput++) {

                // base level based on distance
                var gain = galaxies.audio.DISTANCE_REF /
                (galaxies.audio.DISTANCE_REF +
                    galaxies.audio.DISTANCE_ROLL *
                    (this.distance - galaxies.audio.DISTANCE_REF)); // linear, to match panner algorithm in stereo mix
                if (galaxies.audio.channelAngles[iOutput] !== null) {

                    // exponential falloff function
                    // calculate short distance between the angles
                    var angleDifference = ((galaxies.audio.channelAngles[iOutput] - this.bearing) +
                            galaxies.utils.PI_2 +
                            Math.PI) %
                        galaxies.utils.PI_2 -
                        Math.PI;
                    var directionAttenuation = Math.exp(-angleDifference *
                        angleDifference *
                        galaxies.audio.DIRECTION_FOCUS);
...
galaxies.audio.ObjectSound = function(source, object, baseVolume, loop, start) {
    ...
    this.update = function(delta) {
        ...
        this.sound.source.playbackRate.value = Math.max(galaxies.engine.soundDilation +
            (deltaDistance / galaxies.audio.DOPPLER_FACTOR),
            galaxies.engine.soundDilation * 0.6);
        ...
        for (var iOutput = 0; iOutput < channelMap.length; iOutput++) {
            for (var iSource = 0; iSource < channelMap.length; iSource++) {
                ...
                if (galaxies.audio.channelAngles[channelMap[iSource]] !== null) {
                    ...
                    gain = (Math.cos(galaxies.audio.channelAngles[channelMap[iOutput]] -
                                (galaxies.audio.channelAngles[channelMap[iSource]] + this.angle)) +
                            1) /
                        2;
                }
...
galaxies.audio.toggleTargetMix = function(value) {
    ...
    if ((galaxies.audio.surroundMix)) {
        if (galaxies.audio.audioCtx.destination.maxChannelCount >= 6) {
            galaxies.audio.audioCtx.destination.channelCount = 6;
        }
    } else {
        if (galaxies.audio.audioCtx.destination.maxChannelCount >= 2) {
            galaxies.audio.audioCtx.destination.channelCount = 2;
        }
    }
...
galaxies.audio.setMusicMute = function(mute) {
    if (mute) {
        ...
        galaxies.audio.soundField.volume =
            galaxies.engine.bossMode ? 0.3 : 0.1; // global music volume, should be a const
    }
};