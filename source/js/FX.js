    ...
    var FX = (function() {
        ...
        FX.GetSmallFlameJet = function(longerTail) {
            ...
            var smallFlameJetGroup = new SPE.Group({
                ...
                smallFlameEmitter = new SPE.Emitter({
                    ...
                    color: {
                        value: [
                            new THREE.Color(1, 0.8, 0.2), new THREE.Color(0.7, 0.35, 0.1), new THREE.Color(0.5, 0, 0)
                        ]
                    },
                    angle: { spread: Math.PI },
                    opacity: { value: [1, 0] },
        ...
        FX.ShowExplosion = function(position, style, scale) {
            ...
            if (!FX.toonExplosionPool) {
                FX.toonExplosionPool = new galaxies.Pool(FX.CreatePoolSprite,
                    1,
                    {
        ...
        FX.ShowHit = function(position, type, scale) {
            ...
            if (!FX.poofExplosionPool) {
                FX.poofExplosionPool = new galaxies.Pool(FX.CreatePoolSprite,
                    1,
                    {
                        frames: galaxies.utils.generateSpriteFrames(new THREE.Vector2(0, 0),
        ...
        FX.ShowPlanetExplosion = function() {
            if (!FX.planetExplosion) {
                ...
                var frames_1 =
                        galaxies.utils.generateSpriteFrames(new THREE.Vector2(0, 0),
                            new THREE.Vector2(512, 512),
                    ...
                    sheet = new galaxies.SpriteSheet(texture, frames_1, 30),
        ...
        FX.ShowPowerupAppear = function(position, type) {
            if (!FX.powerupAppearPool) {
                FX.powerupAppearPool = new galaxies.Pool(FX.CreatePoolSprite,
                    1,
                    {
                        frames: galaxies.utils.generateSpriteFrames(new THREE.Vector2(0, 0),
                            new THREE.Vector2(512, 512),
        ...
        FX.ShowStaricles = function(position, type) {
            if (!FX.stariclesGroup) {
                ...
                FX.collectEffectPool = new galaxies.Pool(FX.CreatePoolSprite,
                    1,
                    {
                        frames: galaxies.utils.generateSpriteFrames(new THREE.Vector2(0, 0),
                            new THREE.Vector2(512, 512),
        ...
        FX.UpdateWarpBubble = function() {
            var passes = galaxies.passes,
                ...
                screenAdjust = new THREE.Vector2(galaxies.engine.canvasHalfHeight, galaxies.engine.canvasHalfWidth)
                    .normalize();
        ...
        FX.InitPasses = function() {
            ...
            galaxies.passes.indexes.warpBubble =
                galaxies.engine.composerStack.addPass("WarpBubblePass", false, { progression: 0 });
            galaxies.passes.indexes.focus =
                galaxies.engine.composerStack.addPass("ZoomBlurPass", false, { strength: 0 });
            galaxies.passes.indexes.vignette =
                galaxies.engine.composerStack.addPass("VignettePass", false, { amount: 0 });
            galaxies.passes.indexes.colorAdd =
                galaxies.engine.composerStack.addPass("ColorAddPass", false, { amount: 0 });
        };
