...
galaxies.Projectile.prototype.initialize =
    function(type, model, startAngle, directionOffset, indestructible, particles) {
        this.reset();
...
galaxies.Projectile.prototype.update = function(delta) {
    ...
    if (this.seekTarget) {
        ...
        if (galaxies.utils.flatLengthSqr(seekPos) < galaxies.utils.flatLengthSqr(this.object.position)) {
            ...
            diff = Math.max(Math.min(galaxies.utils.normalizeAngle(targetAngle - this.currentAngle), maxDiff),
                -maxDiff);
