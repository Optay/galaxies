...
galaxies.Obstacle = function(props) {
    ...
    for (var propName in props) {
        if (this.hasOwnProperty(propName)) {
            ...
            console.log("Obstacle attempting to set unknown property.", propName);
        }
    }
...
galaxies.Obstacle.prototype.update = function(delta) {
    if (this.state !== "cinematic") {
        ...
        var speedScale = (galaxies.engine.OBSTACLE_START_DISTANCE - galaxies.engine.PLANET_DISTANCE - this.radius) /
        (galaxies.engine.OBSTACLE_START_DISTANCE -
            galaxies.engine.PLANET_DISTANCE); // normalize to 0-1, 0 at start position, 1 when it reaches the planet
        ...
        case "falling":
            ...
            var innerDist = galaxies.engine.shielded
                ? (galaxies.engine.SHIELD_RADIUS - galaxies.engine.PLANET_DISTANCE)
                : 0;
            ...
            if (distanceToPlanet <= charHeight) {
                ...
                if (distanceToPlanet <= innerDist ||
                    Math.abs(galaxies.utils.normalizeAngle(angle - galaxies.engine.angle)) < 0.35) {
                    this.impact();
...
galaxies.Obstacle.prototype.hit = function(hitPosition, damage, multiply, forceDestroy) {
    if (this.state === "inactive" || this.state === "cinematic") {
        return;
    }
    ...
    angleDiff =
        galaxies.utils.mod((angleDiff + Math.PI), (2 * Math.PI)) - Math.PI; // Convert to minimum angle -Pi to Pi
    ...
    if (forceDestroy || (this.life <= 0)) {
        var innerDistance = galaxies.engine.PROJ_START_Y,
...
galaxies.ObstacleRad.prototype.initModel = function() {
    this.object = new THREE.Mesh(galaxies.resources.geometries["asteroidrad"],
        galaxies.resources.materials["asteroidrad"]);
...
galaxies.ObstacleRadChild.prototype.initModel = function() {
    this.object =
        new THREE.Mesh(galaxies.resources.geometries["asteroid"], galaxies.resources.materials["asteroidrad"]);
